...
from utils import generate_random_anchor_url
...
def parse_url(anchor_url_param: str) -> dict:
    regex = '(?P<endpoint>[api2|enterprise]+)\/anchor\?(?P<params>.*)'
    for match in re.finditer(regex, anchor_url_param):
        return match.groupdict()


def solve_captcha(proxy: str | None,
    ...
    try:
        ...
        if proxy: session.proxies.update({
            'http': proxy,
            'https': proxy
        })

        anchor_url: str = generate_random_anchor_url()

        r = session.get(anchor_url,
                        timeout_seconds=30)

        recaptcha_token: str = BeautifulSoup(r.text, 'lxml').find('input',
                                                                  {
                                                                      'id': 'recaptcha-token'
                                                                  }).get('value')
        ...
        post_data = POST_DATA.format(params["v"], recaptcha_token,
                                     params["k"], params["co"])

        r = session.post('https://www.google.com/recaptcha/api2/reload',
                         params={
                             'k': params['k']
                         },
                         data=post_data,
        ...
        logger.error(f'{account_email} | Ошибка при решении капчи: {error}')

        sleep(2)

        return solve_captcha(proxy=proxy,
